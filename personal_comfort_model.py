...
with st.sidebar:
    ...
    with col2:
        Age = st.selectbox(
            "Age_Category",
            ["18岁以下(0)", "18-30岁(1)", "31-40岁(2)", "41-50岁(3)", "51-60岁(4)", "61岁以上(5)"],
            index=1
        )
...
def generate_data():
    ...
    df = df[feature_order]  # 按照 feature_order 的顺序排列列
    ...
    df = df[['Sex', 'Height', 'Weight', 'Clothing Insulation',
       'Metabolic Rate', 'Indoor Air Temperature', 'Indoor Relative Humidity',
...
with st.expander("📥 Viewing Input Data", expanded=True):
    st.dataframe(df1.style.format("{:.1f}"), height=300)
    st.download_button(
...
if st.button("Start forecasting"):
    try:
        ...
        if hasattr(scaler, 'feature_names_in_'):
            if not (df1.columns == scaler.feature_names_in_).all():
